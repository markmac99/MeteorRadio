...
class MeteorPlotter():
    ...
    def press(self, event):
        ...
        # Move data and audio file to Junk folder. Backspace key moves back to previous file
            ...
            try:
                image_filename = os.path.join(DATA_DIR, os.path.basename(self.file_name.replace("npz", "png")))
    ...
    def plot_specgram(self, Pxx, f, bins, centre_freq, obs_time, flipped=False, utc_time=False, full_frequency_band=False, save_images=False, noplot=False):
        ...
        if full_frequency_band is True:
        ...
        if save_images is True:
            ...
            image_filename = os.path.join(DATA_DIR, 'PSD_' + str(int(centre_freq)) + '_' + obs_time.strftime('_%Y%m%d_%H%M%S_%f.png'))
            print("Saving", image_filename)
            plt.savefig(image_filename)

        #### Displays Z value of mouse cursor ####
        if utc_time is not True:
    ...
    def plot_3dspecgram(self, Pxx, f, bins, centre_freq):
        ...
        if save_images is True:
            ...
            image_filename = os.path.join(DATA_DIR, '/SPG_' + str(int(centre_freq)) + '_' + str(int(sample_rate)) + obs_time.strftime('_%Y%m%d_%H%M%S_%f.png'))
            print("Saving", image_filename)
            plt.savefig(image_filename)
...
def get_observation_data(filename):
    ...
    if len(nameparts) > 5:
        sample_rate = int(nameparts[2])
        obs_date = nameparts[3]
        obs_time = nameparts[4]
        obs_timefrac = nameparts[5]
    else:
        sample_rate = None
        obs_date = nameparts[2]
        obs_time = nameparts[3]
        obs_timefrac = nameparts[4]
...
if __name__ == "__main__":
    ...
    if combine:
        ...
        meteor_plotter.plot_specgram(Pxx, f, bins, centre_freq, obs_times[0], flipped=flipped)
